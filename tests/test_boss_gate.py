...
def test_unactivated_boss_not_treated_as_dead():
    """An unactivated boss (hp=0, hp_max=0) should NOT be auto-recorded as dead.

    Floor bosses are generated with hp=0, hp_max=0 and only get activated
    (scaled up) on first combat encounter. Walking into the boss room before
    activation should NOT record floor_progress or grant free traversal.
    """
    conn = _make_db()
    engine = GameEngine(conn)
    _register(engine)
    ...
    # Set boss to unactivated state (as generated by worldgen)
    conn.execute("UPDATE monsters SET hp = 0, hp_max = 0 WHERE id = 1")
    conn.commit()
    ...
    # floor_progress should NOT be recorded for an unactivated boss
    row = conn.execute(
        "SELECT boss_killed FROM floor_progress WHERE player_id = ? AND floor = 1",
        (player["id"],),
    ).fetchone()
    assert row is None, "Unactivated boss (hp=0, hp_max=0) should not be treated as dead"
